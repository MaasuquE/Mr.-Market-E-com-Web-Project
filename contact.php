<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $message=get_string($conn,$_POST['message']);
        $address=get_string($conn,$_POST['address']);
        $date = date("d M, Y  h:i:s");

        $sql1 = "SELECT * FROM contact_us WHERE email ='{$email}';";
        $res1 = mysqli_query($conn,$sql1);
        if(mysqli_num_rows($res1) > 0){
            $msg="Email already exist. please try another.";
        }else {
            $sql_insrt ="INSERT INTO contact_us(name,email,mobile,comment,added_on,address)
                VALUES('{$name}','{$email}','{$mobile}','{$message}','{$date}','{$address}');";
                            ...
                            <form id="contact-form" action="#" method="post">
                                ...
                                <div class="single-contact-form">
                                    <div class="contact-box message">
                                        <textarea name="message" row="4" placeholder="Your Message"></textarea>
                                    </div>
                                </div>
                                <div class="contact-btn">
                                    <button  type="button " name="submit"   class="fv-btn">Send MESSAGE</button>
                                </div>
